...
@section('main')
    ...
    <div class="container-xl px-4 mt-n10">
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form action="{{ route('user.update', $users->id) }}" id="frmUser" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-3 ms-3 me-3">
                        <label for="name">Name</label>
                        <input class="form-control" id="name" name="name" type="text" placeholder="Name" aria-label="Name" value="{{ $users->name ?? '' }}">
                    </div>
                    <div class="mb-3 ms-3 me-3">
                        <label for="email">Email Address</label>
                        <input class="form-control" id="email" name="email" type="email" placeholder="Email" aria-label="Email" value="{{ $users->email ?? '' }}">
                    </div>
                    <div class="mb-3 ms-3 me-3">
                        <label for="password">Password</label>
                        <input class="form-control" id="password" name="password" type="password" placeholder="Password" aria-label="Password" value="{{ $users->password ?? '' }}">
                    </div>
                    <div class="mb-3 ms-3 me-3">
